 ...
 * Layout: Hero proof statement top | horizontal pipeline timeline | story cards grid
 * Style: Warm gradient — Solarized Light (Ari) blending into Solarized Dark (Ivan)
 * Typography: Bold display for hero, Inter body, mono for metrics
 * Density: Medium — stories take center stage
 */
...
function StoryCard({ story }: { story: (typeof stories)[0] }) {
  ...
  const pct = (completedStages / totalStages) * 100;
...
function PipelineTimeline() {
  ...
  return (
    ...
    >
      <div className="text-xs font-mono-data uppercase tracking-widest mb-4" style={{ color: "#586e75" }}>
        The Loop: customer signal → shipped release
      </div>
        ...
        {stages.map((stage, i) => (
          <div key={stage.key} className="flex items-center">
            <div className="flex flex-col items-center gap-1.5">
              <div
                className="w-10 h-10 rounded-full flex items-center justify-center border-2 text-base"
                style={{
              ...
              <div className="text-center">
                <div className="text-xs font-medium" style={{ color: "#eee8d5" }}>
                  {stage.label}
                </div>
                <div
                  className="text-xs font-mono-data"
                  style={{
                    color: stage.actor === "Ari" ? "#2aa198" : stage.actor === "Ivan" ? "#268bd2" : "#586e75",
                  }}
                >
                  {stage.actor}
                </div>
            ...
            {i < stages.length - 1 && (
              <div
                ...
                style={{ backgroundColor: "#586e75", minWidth: 24 }}
              />
            )}
          </div>
        ))}
  ...
  return (
    <div className="flex items-center gap-6 px-6 py-3" style={{ backgroundColor: "#073642" }}>
      <span
        ...
        style={{ color: accentColor, minWidth: 32 }}
      >
        {label}
      </span>
      {kpis.map((kpi) => (
    ...
    >
        ...
        <nav className="flex items-center gap-4">
          {["Ari", "Ari ↔ Ivan", "Ivan"].map((tab) => (
            <button
              ...
              style={{
                color: tab === "Ari ↔ Ivan" ? "#eee8d5" : "#586e75",
                borderBottom: tab === "Ari ↔ Ivan" ? "2px solid #268bd2" : "2px solid transparent",
                paddingBottom: 2,
              }}
            >
              {tab}
      ...
      >
        ...
        <p className="text-base" style={{ color: "#839496" }}>
          ...
          <span className="font-mono-data" style={{ color: "#859900" }}>
            {summaryStats.loopClosedPct}%
          </span>{" "}
          loop closed · avg{" "}
          <span className="font-mono-data" style={{ color: "#eee8d5" }}>
      ...
      <div className="flex flex-1 gap-0">
        ...
        >
          <div
            className="px-4 py-3 border-b font-mono-data text-xs uppercase tracking-widest"
            style={{ borderColor: "#586e75", color: "#586e75" }}
          >
            Live Handoff Feed
          </div>
